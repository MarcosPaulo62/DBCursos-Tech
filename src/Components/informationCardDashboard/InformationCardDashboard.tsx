import {StyledInformationCardDashboard, StyledInformationCardDashboardContainer} from "./style";
...
export default function InformationCardDashboard({type, id}: InformationCardDashboardProps) {
  const {listaAlunoById, alunoById, deleteAlunoById} = useContext(AlunosContext);
  const {listaCursoById, cursoById, deleteCursoById} = useContext(CursosContext);
  const {listaProfessorById, professorById, deleteProfessorById} = useContext(ProfessoresContext);
  ...
  useEffect(() => {
    ...
    type === "curso" && cursoById?.idProfessor && listaProfessorById(cursoById.idProfessor.toString());
  }, []);
  ...
  function removeElement() {
    ...
    type === "professor" && deleteProfessorById(professorById?.idProfessor.toString());
    ...
    <>
      <StyledInformationCardDashboardContainer>
        ...
        <StyledInformationCardDashboard>
          <img src={type === "aluno" ? student : type === "professor" ? teacher : course} alt="" />
          <div>
            <StyledSpan className="title-information" fontSize="lg">
              <strong>{type === "aluno" ? "Aluno" : type === "professor" ? "Professor" : "Curso"}:</strong>{" "}
            ...
            {localStorage.getItem("cargo") === "admin" && (
              <div className="information-periodCh">
                <StyledSpan className="block-span" fontSize="lg">
                  <strong>{type === "aluno" ? "Idade" : type === "professor" ? "Salário" : "Turno"}:</strong>{" "}
                  {type === "aluno" && (alunoById?.idade ?? "Não informado")}
                  {type === "curso" && (periodoCurso(cursoById?.periodo) ?? "Não informado")}
                  {type === "professor" &&
                ...
                <StyledSpan className="block-span" fontSize="lg">
                  <strong>{type === "aluno" ? "CPF" : type === "professor" ? "CPF" : "Carga horária"}:</strong>{" "}
            ...
            {type !== "professor" && (
              <StyledSpan className="professor-tag" fontSize="lg">
                <strong>{type === "aluno" ? "Email" : "Professor"}:</strong> {type === "aluno" && alunoById?.email}
                {type === "curso" && nomeProfessorById()}
              </StyledSpan>
            )}
            <StyledSpan className="description" fontSize="lg">
              <strong>{type === "aluno" ? "Matrícula" : type === "professor" ? "Especialidade" : "Descrição"}:</strong>
            </StyledSpan>
            <StyledTag className="description">
              {type === "aluno" && (alunoById?.numeroDeMatricula ?? "Não informado")}
              {type === "curso" && (cursoById?.descricao ?? "Não informado")}
              {type === "professor" && (professorById?.especialidade ?? "Não informado")}
            </StyledTag>
            {localStorage.getItem("cargo") === "admin" && (
